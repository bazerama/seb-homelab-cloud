    ...
    case $choice in
        1)
            ...
            if [ -f .env ] && grep -q AWS_ACCESS_KEY_ID .env 2>/dev/null; then
                ACCESS_KEY_ID=$(grep 'AWS_ACCESS_KEY_ID=' .env | cut -d'=' -f2 | tr -d '"' | tr -d "'" | xargs)
                SECRET_KEY=$(grep 'AWS_SECRET_ACCESS_KEY=' .env | cut -d'=' -f2 | tr -d '"' | tr -d "'" | xargs)
                if [ -n "$ACCESS_KEY_ID" ] && [ -n "$SECRET_KEY" ]; then
                    echo -e "${GREEN}✓${NC} Loaded existing credentials from .env"
                fi
            fi
            ...
            if [ -z "$ACCESS_KEY_ID" ] || [ -z "$SECRET_KEY" ]; then
                ...
                echo "   Find them in OCI Console → Profile icon → User settings"
                echo "   → Tokens and keys → Customer Secret Keys"
                echo ""
                read -p "Paste your Access Key (AWS_ACCESS_KEY_ID): " ACCESS_KEY_ID
                if [ -z "$ACCESS_KEY_ID" ]; then echo -e "${RED}Error: Access Key is required.${NC}"; exit 1; fi
                echo ""
                read -p "Paste your Secret Key (AWS_SECRET_ACCESS_KEY): " SECRET_KEY
                if [ -z "$SECRET_KEY" ]; then echo -e "${RED}Error: Secret Key is required.${NC}"; exit 1; fi
            fi
            echo ""
            ;;
        2)
...
# Step 5: Create .env file (if credentials were generated)
if [ -n "${ACCESS_KEY_ID:-}" ] && [ -n "${SECRET_KEY:-}" ]; then
...
EOF
    ...
    chmod 600 .env
    echo -e "${GREEN}✓${NC} .env created (add to .gitignore)"
    echo ""
fi
...
if [ -n "${ACCESS_KEY_ID:-}" ]; then
    echo "1. Credentials saved to .env (loaded automatically by make)"
    echo ""
fi
